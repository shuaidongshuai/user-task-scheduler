...
- `status`：`PENDING/RUNNABLE/RUNNING/WAIT_HOLD/WAIT_RETRY/SUCCESS/FAILED/CANCELLED`
- `priority`：优先级，范围 `0~99`，数值越小优先级越高
- `execute_at` / `next_retry_at`：计划执行时间 / 下次重试时间
- `retry_count` / `max_retry_count`：当前重试次数 / 最大重试次数
- `hold_round_count` / `hold_max_rounds` / `hold_retry_delay_sec`：WAIT_HOLD 已执行轮次 / 最大轮次 / 每轮等待秒数
...
- `execute_no`：一次执行一次号，唯一索引；首次执行时也作为 task lease 值
- `task_id` / `task_no` / `group_code` / `user_id`
- `status`：`RUNNING/WAIT_HOLD/SUCCESS/FAILED/WAIT_RETRY`
...
stateDiagram-v2
    ...
    WAIT_HOLD --> RUNNING: acquireLeaseOnly + DB CAS + worker submit
    WAIT_RETRY --> RUNNING: tryAcquire + DB CAS + worker submit
    RUNNING --> SUCCESS: 执行成功
    RUNNING --> WAIT_HOLD: 外部异步任务等待下一轮轮询
...
- `WAIT_HOLD -> RUNNING` 不再重复增加并发计数，只单独获取本轮 task lease 后再执行 DB CAS。
- `RUNNING` 与 `WAIT_HOLD` 都表示任务已占用 group/user 并发额度。
- 等待超时只作用于 `PENDING/RUNNABLE/WAIT_RETRY/WAIT_HOLD`，不能误杀已进入业务执行中的 `RUNNING`。
...
- group/user running 计数代表“当前占用并发槽位的任务数”
- `WAIT_HOLD` 任务仍计入 running
- task lease 代表“当前这一轮由哪个实例执行”，WAIT_HOLD 每一轮恢复执行时会重新获取 lease，但不重复增加 running 计数
...
6. 如提交时显式传入 `holdMaxRounds/holdRetryDelaySec`，将最终生效值落入任务表；未传则使用全局默认值。
...
4. 只读取该 user、该优先级下的一页任务：
   - `pageSize = dispatchBatchSize`
5. 对这一页逐个任务执行：
   - 校验 DB 状态 / 到期时间 / wait deadline
   - 业务状态短路检查
   - 普通任务：`tryAcquire -> casToRunning`
   - `WAIT_HOLD` 任务：`acquireLeaseOnly -> casWaitHoldToRunning`
...
### 9.5 executeSync 与 WAIT_HOLD 的语义

`executeSync` 仍保留原方法名，但其语义需要明确为“同步提交，并在当前线程执行首轮”：

- `submitDirect` 会先完成：
  - 任务落库
  - group/user 并发抢占
  - `RUNNABLE -> RUNNING` 的 DB CAS
- 之后由 `workerService.executeDirect(...)` 在当前线程执行首轮 handler

首轮执行后的返回语义：

- 若任务首轮结束为 `SUCCESS`，`executeSync(...)` 正常返回
- 若任务首轮返回 `WAIT_HOLD`，也视为本次同步提交成功，`executeSync(...)` 正常返回
- 但 `WAIT_HOLD` 返回只表示：
  - 任务已成功进入后续轮询流程
  - group/user 并发已被该任务占用
  - 后续仍会由框架按 `WAIT_HOLD` 状态机继续调度执行
- 因此，`executeSync(...)` 返回不再等价于“任务整体生命周期已经结束”
- 若首轮结束为 `FAILED`、`WAIT_RETRY` 等非成功态，则 `executeSync(...)` 仍抛异常

设计边界：

- `WAIT_HOLD` 是业务主动选择的“运行中挂起继续轮询”，可视为本次同步提交成功
- `WAIT_RETRY` 是失败后的框架重试，不视为同步提交成功
...
`WAIT_HOLD` 恢复执行时，不调用 `tryAcquire`，而是调用 lease-only 接口：

1. 对 `sched:task:lease:{taskId}` 执行 `SET NX EX`
2. 不修改 group/user running 计数
3. 只为当前这一轮执行建立心跳续租目标
...
6. finally 中：
   - 取消 heartbeat
   - `WAIT_HOLD`：仅释放本轮 task lease，不释放 running 计数
   - 其他终态/重试：按 `executeNo` 释放 Redis 占用
   - 更新执行明细最终状态
7. 如果 release mismatch，不盲目扣减 running，触发即时 reconcile

`WAIT_HOLD` 业务语义：

- handler 返回 `TaskExecuteResult.waitHold()`
- 框架自动：
  - `hold_round_count + 1`
  - 校验是否超过 `hold_max_rounds`
  - `execute_at = now + hold_retry_delay_sec`
  - 状态置为 `WAIT_HOLD`
  - 重新进入 time queue
- `ext_info` 仍完全由业务自行维护，框架只持久化透传
...
2. 定时任务扫描 `PENDING/RUNNABLE/WAIT_RETRY/WAIT_HOLD` 中已超时任务
...
2. 查询 `RUNNABLE/WAIT_RETRY/WAIT_HOLD`
3. 查询未来 `PENDING`
...
| worker submit 失败 | 普通任务回滚为 `RUNNABLE`；`WAIT_HOLD` 恢复任务回滚为 `WAIT_HOLD` | 普通任务按 `executeNo` release；`WAIT_HOLD` 恢复任务仅 release lease | 重新入 time queue |
| 业务状态已成功/失败 | 直接终态化 | 不占用 running | 从 ready 删除 |
| 业务状态未就绪 | 延后 `execute_at` | 不占用 running | 重新入 time queue |
| handler 返回 `WAIT_HOLD` | `WAIT_HOLD` | 保留 running，仅 release 本轮 lease | 入 time queue |
